...
/**
 * Servlet implementation class cztevtclean
 */
@WebServlet("/cztevtclean")
public class cztevtclean extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String outputevtfile=request.getParameter("outputevtfile");
		System.out.println("outputevtfile========"+outputevtfile);
		
		
		String alpha=request.getParameter("alpha");
		System.out.println("alpha======"+alpha);
		
		String veto=request.getParameter("veto");
	     ...
	     for(File f: files){
	 	  ...
	 	  if (uname.equals(part[1]))
	 	  {
	 		 ...
	 		 for(File f1: files1){
				...
				if( (lastWord.endsWith("level2_quad_bc_ds_pc.evt"))||(lastWord.endsWith("level2_common_bc_ds_pc.evt")))
				{
	     ...
	     Cookie cookie = null;
	      ...
	      if( cookies != null ) {
	          ...
	          for (int i = 0; i < cookies.length; i++) {
	             ...
	             if (cookie.getValue( ).endsWith("level2_quad_clean.evt"))
	             {
	            	 outputevtfile=cookie.getValue( );
	            	 outputevtfile=path1+outputevtfile;
	            	 System.out.println("evt-----------------------------------------------------"+outputevtfile);
	             }
	             if (cookie.getName( ).equals("alpha"))
	             {
	            	 alpha=cookie.getValue( );
	            	
	            	 System.out.println("evt-----------------------------------------------------"+alpha);
	             }
	             if (cookie.getName( ).equals("veto"))
	             {
	            	 veto=cookie.getValue( );
	            	
	            	 System.out.println("evt-----------------------------------------------------"+veto);
	             }
		...
		{		
	        ...
	        String status;
	        String s1=null;
	        Process p = Runtime.getRuntime().exec("cztevtclean "+eventfile+" "+outputevtfile+" "+alpha+" "+veto+"  "+history+" "+write+"");
	       //Process p = Runtime.getRuntime().exec("python2 uvit9pt.py "+TARGET+" "+RA+" "+DEC+"");
			...
			//	fileWriter1.write(status);			
				...
				if (status.contains("CZTEVTCLEAN COMPLETED SUCCESSFULLY"))
				{
					flag=1;
					String pathformove="cp "+outputevtfile+" "+homedir+"/level2files/";
				...
				if (flag == 1)
				{
					try {	   	          
			   	           ...
			   	           message.setSubject("CZTEVTCLEAN COMPLETED SUCCESSFULLY"); 	          
			   	           message.setText("CZTEVTCLEAN COMPLETED SUCCESSFULLY");       
		...
		//	fileWriter1.write(status);			
			...
			System.out.println("OutputstringBuffer\n"+s1);
     	    ...
     	    if (m.find())
     	    {
     	      ...
     	      System.out.println("CZTEVTCLEAN COMPLETED SUCCESSFULLY");
     	     buffer.append("<filepath>" +"CZTEVTCLEAN COMPLETED SUCCESSFULLY"+ "</filepath>\n");
     	    for(File f: files){
     	    	if (!f.isDirectory())
     	           f.delete();
     	    }
